...
const ProductList: React.FC<ProductListProps> = ({ products, handleBuyProduct, refetchProducts, isLoading }) => {
    ...
    const [ensNames, setEnsNames] = useState<{ [address: string]: string }>({}); // State to hold ENS names

    useEffect(() => {
        const resolveENSNames = async () => {
            try {
                const provider = ethers.getDefaultProvider(); // You can specify a provider if needed
                const addressesToResolve = Array.from(new Set(
                    products.flatMap(product => [product.creator, product.buyer])
                ));

                const ensPromises = addressesToResolve.map(async (addr) => {
                    const name = await provider.lookupAddress(addr);
                    return { addr, name };
                });

                const ensResults = await Promise.all(ensPromises);
                const newEnsNames: { [address: string]: string } = {};
                ensResults.forEach(({ addr, name }) => {
                    if (name) {
                        newEnsNames[addr] = name;
                    }
                });
                                ...
                                <p className="text-sm text-n4">
                                    ...
                                    >
                                        <span className="text-p1">
                                            {ensNames[product.creator] || truncateAddress(product.creator)}
                                        </span>
                                    </a>
                                </p>
                                <div
                                ...
                                {!product.isSold ? (
                                    ...
                                    <div className="flex justify-center">
                                        <button className="text-sm px-6 py-3.5 rounded-full button bg-n2 text-n4 font-medium cursor-not-allowed" disabled>
                                            <p>Bought By: {ensNames[product.buyer] || truncateAddress(product.buyer)}</p>
                                        </button>
                                    </div>
                                )}
